...
//                    recvScanData.interrupt();
    ...
    private void showBleConnectDialog(Device device) {
        ...
        // Connect button click - Steps 2-4
        btnConnect.setOnClickListener(v -> {
            ...
            new Thread(() -> {
                // Execute Steps 2-4: Master Mode → Connect
                ...
                runOnUiThread(() -> {
                    ...
                    if (result.isSuccess()) {
                        ...
                        logBuilder.append("=== Connection Successful ===\n");
                        logBuilder.append("MAC: ").append(device.getMacAddress()).append("\n");
                        logBuilder.append("Handle: ").append(result.getHandle()).append("\n");
                        logBuilder.append("Status: Ready to send data\n\n");
                        ...
                        logBuilder.append("=== Connection Failed ===\n");
        ...
        btnSend.setOnClickListener(v -> {
            ...
            new Thread(() -> {
                // Execute Steps 5-9: UUID Scan → Check Handle → Set TRX → Set TTM → Send Data
                BleConnection.SendResult result = bleConnection.sendDataComplete(sendData, 3000);
                ...
                runOnUiThread(() -> {
                    ...
                    if (result.isSuccess()) {
                        logBuilder.append("=== Data Send Successful ===\n");
                        logBuilder.append("TX: ").append(sendData).append("\n");
                        logBuilder.append("Length: ").append(sendData.length()).append(" bytes\n\n");
                        etSendData.setText("");
                    } else {
                        logBuilder.append("=== Data Send Failed ===\n");
                        logBuilder.append("Error: ").append(result.getError()).append("\n\n");
                    }
                    tvReceivedLog.setText(logBuilder.toString());
                });
...
//                }
        ...
        // Disconnect button click - Step 10
        btnDisconnect.setOnClickListener(v -> {
            progressConnection.setVisibility(View.VISIBLE);

            new Thread(() -> {
                // Execute Step 10: Disconnect
                ...
                runOnUiThread(() -> {
                    ...
                    if (success) {
                        ...
                        logBuilder.append("=== Disconnected Successfully ===\n\n");
                        tvReceivedLog.setText(logBuilder.toString());
                    } else {
